 ...
 * writing or not all bits will be written. This class is intended to be used
 * with <code>BitInputStream</code> to facilitate reading and writing data in a
 * bits-at-a-time manner.
 * <P>
 ...
 * objects so client code does not have to catch or rethrow them.
 * <P>
 * 
...
public class BitOutputStream extends OutputStream {
	 ...
	 * @throws RuntimeException if opening file fails for either FileNotFound or for
	 *                          Security exceptoins
	 */
	 ...
	 * written. Either this function or flush must be called or not all bits will be
	 * written
	 * 
	...
	public void writeBits(int howManyBits, int value) {
		...
		while (howManyBits >= myBitsToGo) {
			myBuffer = (myBuffer << myBitsToGo) | (value >>> (howManyBits - myBitsToGo));
			try {
				write(myBuffer);
			} catch (java.io.IOException ioe) {
